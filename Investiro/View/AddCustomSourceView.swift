    ...
    var body: some View {
        Form {
            Section(header: Text(StringManager.shared.get("namn"))) {
                TextField(StringManager.shared.get("namn"), text: $name)
            }

            Section(header: Text("RSS URL")) {
                TextField(StringManager.shared.get("angerss"), text: $urlString)
                    .keyboardType(.URL)
        ...
        .navigationTitle(StringManager.shared.get("läggtillkälla"))
    }
}
